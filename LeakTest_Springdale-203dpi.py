...
#FOR TROUBLESHOOTING OR QUESTIONS EMAIL CJ@AMERICANTUBING.COM
...
#time.sleep(20) #This has been added at Daniel's request.  I don't know if the process there differs from what I have here in Sringdale, 
                #but it's not needed in Springdale on my test bench.
...
def CreateBarcode():
    ...
    while True:
        ...
        if GPIO.event_detected(ULsigPass):
                ...
                height = 4
                l.origin(16,height)
                l.write_text('ATI PN: ' + str(ATIPart), char_height=2, char_width=2, line_width=30, justification='L')
                l.endorigin()

                height += 2
                l.origin(16,height)
                l.write_text('ATI WO: ' + str(WorkOrder), char_height=2, char_width=2, line_width=30, justification='L')
                l.endorigin()

                height += 2
                l.origin(16,height)
                l.write_text('Cust PN: ' + CustPart, char_height=2, char_width=2, line_width=30, justification='L')
                l.endorigin()
                ...
                height += 2
                l.origin(16, height)
                l.write_text(datetime.now().isoformat(sep='/', timespec='minutes'), char_height=2, char_width=2, line_width=30, justification='L')
                l.endorigin()

                height = 4
                l.origin(30,height)
                l.write_text('ATI PN: ' + str(ATIPart), char_height=2, char_width=2, line_width=30, justification='R')
                l.endorigin()

                height += 2
                l.origin(30,height)
                l.write_text('ATI WO: ' + str(WorkOrder), char_height=2, char_width=2, line_width=30, justification='R')
                l.endorigin()

                height += 2
                l.origin(30,height)
                l.write_text('Cust PN: ' + CustPart, char_height=2, char_width=2, line_width=30, justification='R')
                l.endorigin()
                ...
                height += 2
                l.origin(30, height)
                l.write_text(datetime.now().isoformat(sep='/', timespec='minutes'), char_height=2, char_width=2, line_width=30, justification='R')
                l.endorigin()
                ...
                sys.stdout = old_stdout
                zpl_convert = zpl_convert.replace('BQN,2,3,Q,7^FD', 'BXN,4,200,0,0,1,_^FH_^FD')
                print('Raw info sent to printer:\n', (zpl_convert)) #Shows what ZPL data is sent to the printer
            
                mysocket = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
                host = "192.168.5.192" #Springdale dev printer
                port = 9100   
                try:           
